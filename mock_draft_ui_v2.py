...
def _handle_actions(state: dict[str, Any]) -> None:
    ...
    if did:
        pid = str(did)
        on = state.get("status") == "active" and not state.get("paused") and state.get("currentTeam") == state.get("userTeamId")
        if on and get_player(state, pid):
...
def _position_chips(selected: str) -> None:
    bits = []
    for pos in ("QB", "RB", "WR", "TE", "FLEX", "K", "D/ST"):
        c1, c2 = _colors(pos)
        label = "DEF" if pos == "D/ST" else pos
        active = " active" if selected == pos else ""
        href = f'?page=Mock%20Draft&draft_tab=DRAFT_BOARD&draft_view=LIST&pos={quote(pos)}'
...
def _status_bar_inner(state: dict[str, Any]) -> None:
...
def _player_list(state, history, roi, rankings, weekly, ask_shiva_func, api_key):
    query_pos = str(st.query_params.get("pos") or "").upper()
    if query_pos in {"QB", "RB", "WR", "TE", "D/ST", "K"}:
        st.session_state["mock2_pos"] = query_pos
    ...
    with c2:
        pos = st.selectbox("Position", ["ALL", "QB", "RB", "WR", "TE", "D/ST", "K"], key="mock2_pos", label_visibility="collapsed", format_func=lambda x: "All Positions" if x == "ALL" else x)
    teams = ["ALL"] + sorted({str(p.get("team")) for p in state.get("availablePlayers", []) if p.get("team")})
    with c3:
        team_filter = st.selectbox("Team", teams, key="mock2_team", label_visibility="collapsed", format_func=lambda x: "All Teams" if x == "ALL" else x)
    with c4:
        st.markdown('<a class="mock-filter-button" href="?page=Mock%20Draft&draft_tab=DRAFT_BOARD&draft_view=LIST" target="_self" title="Reset filters">▽</a>', unsafe_allow_html=True)
    _position_chips(pos)
    ...
    if pos != "ALL":
        ...
        pool = [p for p in pool if (p.get("position") in flex_positions if pos == "FLEX" else p.get("position") == pos)]
    if team_filter != "ALL":
    ...
    rq = _return_query("LIST")
    for p in pool:
        c1c, c2c = _colors(str(p.get("position")))
        pid = str(p["id"])
        name = str(p.get("name"))
        team = html.escape(str(p.get("team") or "—"))
        pt = html.escape(str(p.get("position") or "—"))
        adp = float(p.get("adp") or 0)
        rank = int(p.get("rank") or 0)
        link = player_link_html(pid, name, css_class="mock-name", return_page="Mock Draft", return_query=rq)
        qc = "mock-q on" if pid in queued else "mock-q"
        qh = f'?page=Mock%20Draft&draft_tab=DRAFT_BOARD&draft_view=LIST&queue={quote(pid)}'
        pick = f'<a class="mock-pick on" href="?page=Mock%20Draft&draft_tab=DRAFT_BOARD&draft_view=LIST&draft={quote(pid)}" target="_self">PICK</a>' if on else '<span class="mock-pick">PICK</span>'
    ...
    if on and st.button("🤖 WHO SHOULD I PICK?", key="mock2_shiva_btn", use_container_width=True, type="primary"):
...
def _board(state: dict[str, Any]) -> None:
    ...
    st.markdown(f'<div class="board-top"><div></div><div><div class="board-title">MOCK DRAFT BOARD</div><div class="board-round">Round {cr}⌄</div></div><div class="board-meta">{teams} Teams • {html.escape(str(state["settings"].get("scoring", "PPR")))}</div></div>', unsafe_allow_html=True)
    legend = []
    for pos in ("QB", "RB", "WR", "TE", "FLEX", "K", "D/ST"):
        c1, _ = _colors(pos)
        label = "DEF" if pos == "D/ST" else pos
    ...
    for rnd in range(1, rounds + 1):
        for col in range(1, teams + 1):
            ...
            if pick:
                c1c, c2c = _colors(str(pick.get("position")))
                pid = str(pick.get("playerId"))
                pname = str(pick.get("playerName"))
                pos = str(pick.get("position"))
                link = player_link_html(pid, _compact_name(pname), css_class="pick-name", return_page="Mock Draft", return_query=_return_query("BOARD"))
                parts.append(f'<div class="pick-card" style="background:linear-gradient(145deg,{c1c},{c2c})"><div class="pick-no">{rnd}.{rp}</div>{link}<div class="pick-pos">{html.escape(pos)}</div></div>')
            else:
...
def _queue(state: dict[str, Any]) -> None:
    ...
    for pid in list(state.get("queue") or []):
        ...
        if not p:
            continue
        link = player_link_html(str(p["id"]), str(p["name"]), css_class="mock-name-plain", return_page="Mock Draft", return_query="draft_tab=QUEUE")
        st.markdown(f'<div class="queue-row"><div class="slot">{html.escape(str(p.get("position")))}</div><div>{link}</div><a class="mock-q on" href="?page=Mock%20Draft&draft_tab=QUEUE&queue={quote(str(pid))}" target="_self">×</a></div>', unsafe_allow_html=True)
...
def _team(state: dict[str, Any]) -> None:
    ...
    for slot, player in roster_slots(state, state["userTeamId"]):
        body = player_link_html(str(player["id"]), str(player["name"]), css_class="mock-name-plain", return_page="Mock Draft", return_query="draft_tab=TEAM") if player else '<span style="color:#75808a">—</span>'
        pos = html.escape(str(player.get("position"))) if player else ""
        st.markdown(f'<div class="team-row"><div class="slot">{html.escape(slot)}</div><div>{body}</div><div class="mock-cell">{pos}</div></div>', unsafe_allow_html=True)
...
def _results(state: dict[str, Any]) -> None:
    ...
    for pick in state.get("picks", []):
        link = player_link_html(str(pick.get("playerId")), str(pick.get("playerName")), css_class="mock-name-plain", return_page="Mock Draft", return_query="draft_tab=RESULTS")
        pick_in_round = ((int(pick.get("pickNumber", 1)) - 1) % int(state["settings"]["teamsCount"])) + 1
        st.markdown(f'<div class="result-row"><div class="slot">{int(pick.get("round", 0))}.{pick_in_round:02d}</div><div>{link}</div><div class="mock-cell">{html.escape(str(pick.get("position")))}</div></div>', unsafe_allow_html=True)


def _settings(state: dict[str, Any]) -> None:
    s = state["settings"]
    st.markdown(f'<div class="settings-card"><div class="section-title">DRAFT SETTINGS</div><div class="settings-line">{int(s["teamsCount"])} Teams • {html.escape(str(s["scoring"]))} • Snake Draft • {int(s["rounds"])} Rounds • {int(s["secondsPerPick"])} sec/pick</div></div>', unsafe_allow_html=True)
    c1, c2 = st.columns(2, gap="small")
    with c1:
        if state.get("paused"):
            if st.button("RESUME DRAFT", use_container_width=True, key="resume_draft"):
                resume_draft(state)
                st.rerun()
        else:
            if st.button("PAUSE DRAFT", use_container_width=True, key="pause_draft"):
                pause_draft(state)
                st.rerun()
    with c2:
...
def render_mock_draft_room_v2(rankings: pd.DataFrame, weekly: pd.DataFrame, history: pd.DataFrame, roi: pd.DataFrame, db_path, ask_shiva_func, api_key: str | None) -> None:
    _css()
    settings_open = str(st.query_params.get("draft_settings") or "") == "1"
    gear_href = '?page=Mock%20Draft' if settings_open else '?page=Mock%20Draft&draft_settings=1'
    st.markdown(f'<div class="mock-head"><a href="?page=Home" target="_self">‹</a><div><div class="mock-title">MOCK DRAFT</div><div class="mock-sub">10-Team PPR • Snake Draft</div></div><a class="mock-gear" href="{gear_href}" target="_self">⚙</a></div>', unsafe_allow_html=True)
    ...
    if state.get("status") == "active" and not state.get("paused") and state.get("currentTeam") != state.get("userTeamId"):
        advance_cpu_until_user(state)
    st.session_state[STATE_KEY] = state
    _handle_actions(state)
    if settings_open:
        _settings(state)
    tab = str(st.query_params.get("draft_tab") or st.session_state.get("mock2_tab", "DRAFT_BOARD")).upper()
    if tab not in {"DRAFT_BOARD", "QUEUE", "TEAM", "RESULTS"}:
        tab = "DRAFT_BOARD"
    st.session_state["mock2_tab"] = tab
    labels = (("DRAFT_BOARD", "DRAFT BOARD"), ("QUEUE", "QUEUE"), ("TEAM", "TEAM"), ("RESULTS", "RESULTS"))
    st.markdown('<div class="mock-tabs">' + ''.join(f'<a class="mock-tab{" active" if tab == key else ""}" href="?page=Mock%20Draft&draft_tab={key}" target="_self">{label}</a>' for key, label in labels) + '</div>', unsafe_allow_html=True)
    if tab == "DRAFT_BOARD":
        view = str(st.query_params.get("draft_view") or st.session_state.get("mock2_view", "LIST")).upper()
        if view not in {"LIST", "BOARD"}:
            view = "LIST"
        st.session_state["mock2_view"] = view
        st.markdown('<div class="draft-view-toggle"><a class="draft-view' + (' active' if view == 'LIST' else '') + '" href="?page=Mock%20Draft&draft_tab=DRAFT_BOARD&draft_view=LIST" target="_self">PLAYER LIST</a><a class="draft-view' + (' active' if view == 'BOARD' else '') + '" href="?page=Mock%20Draft&draft_tab=DRAFT_BOARD&draft_view=BOARD" target="_self">BOARD VIEW</a></div>', unsafe_allow_html=True)
        if view == "LIST":
