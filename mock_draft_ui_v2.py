...
def _position_chips(selected):
    ...
    for pos in ("QB","RB","WR","TE","FLEX","K","D/ST"):
        c1,c2=_colors(pos); label="DEF" if pos=="D/ST" else pos; active=" active" if selected==pos else ""; href=f'?page=Mock%20Draft&draft_tab=PLAYERS_AVAILABLE&pos={quote(pos)}'
...
def _player_list(state,history,roi,rankings,weekly,ask_shiva_func,api_key):
    ...
    with c4: st.markdown('<a class="mock-filter-button" href="?page=Mock%20Draft&draft_tab=PLAYERS_AVAILABLE&reset_filters=1" target="_self" title="Reset filters">▽</a>',unsafe_allow_html=True)
    ...
    st.markdown('<div class="mock-list-head"><div>RK</div><div>PLAYER</div><div>POS</div><div>TEAM</div><div style="text-align:right">ADP</div><div>QUEUE</div><div></div></div>',unsafe_allow_html=True)
    on=state.get("status")=="active" and not state.get("paused") and state.get("currentTeam")==state.get("userTeamId"); queued=set(state.get("queue") or []); rq=_return_query("PLAYERS_AVAILABLE")
    for p in pool:
        c1c,c2c=_colors(p.get("position")); pid=str(p["id"]); name=str(p.get("name")); team=html.escape(str(p.get("team") or "—")); pt=html.escape(str(p.get("position") or "—")); adp=float(p.get("adp") or 0); rank=int(p.get("rank") or 0); link=player_link_html(pid,name,css_class="mock-name",return_page="Mock Draft",return_query=rq); qc="mock-q on" if pid in queued else "mock-q"; qlabel="ADDED" if pid in queued else "ADD"; qh=f'?page=Mock%20Draft&draft_tab=PLAYERS_AVAILABLE&queue={quote(pid)}'; pick=f'<a class="mock-pick on" href="?page=Mock%20Draft&draft_tab=PLAYERS_AVAILABLE&draft={quote(pid)}" target="_self">PICK</a>' if on else '<span class="mock-pick">PICK</span>'
        st.markdown(f'<div class="mock-player" style="background:linear-gradient(90deg,{c2c},{c1c})"><div class="mock-rank">{rank}</div><div>{link}</div><div class="mock-cell">{pt}</div><div class="mock-cell">{team}</div><div class="mock-adp">{adp:.1f}</div><a class="{qc}" href="{qh}" target="_self">{qlabel}</a>{pick}</div>',unsafe_allow_html=True)
...
def _board(state):
    ...
    for rnd in range(1,rounds+1):
        for col in range(1,teams+1):
            ...
            if pick:
                c1c,c2c=_colors(pick.get("position")); pid=str(pick.get("playerId")); pname=str(pick.get("playerName")); pos=str(pick.get("position")); href=player_profile_href(pname,pid,return_page="Mock Draft",return_query=_return_query("DRAFT_BOARD")); link=f'<a class="pick-name" href="{html.escape(href,quote=True)}" target="_self">{html.escape(_compact_name(pname))}</a>'; parts.append(f'<div class="pick-card" style="background:linear-gradient(145deg,{c1c},{c2c})"><div class="pick-no">{rnd}.{rp}</div>{link}<div class="pick-pos">{html.escape(pos)}</div></div>')
...
def _roster(state):
    ...
    for slot,p in roster_slots(state,state["userTeamId"]):
        body=player_link_html(str(p["id"]),str(p["name"]),css_class="mock-name-plain",return_page="Mock Draft",return_query="draft_tab=ROSTER") if p else '<span style="color:#75808a">—</span>'; pos=html.escape(str(p.get("position"))) if p else ""; st.markdown(f'<div class="team-row"><div class="slot">{html.escape(slot)}</div><div>{body}</div><div class="mock-cell">{pos}</div></div>',unsafe_allow_html=True)
...
def render_mock_draft_room_v2(rankings:pd.DataFrame,weekly:pd.DataFrame,history:pd.DataFrame,roi:pd.DataFrame,db_path,ask_shiva_func,api_key:str|None)->None:
    ...
    if state is None:
        ...
        with st.form("mock2_setup"):
            ...
            if st.form_submit_button("START MOCK DRAFT",use_container_width=True,type="primary"): state=initialize_draft(pool,int(teams),int(slot),scoring,DEFAULT_ROSTER.copy(),int(rounds),90); start_draft(state); advance_cpu_until_user(state); st.session_state[STATE_KEY]=state; st.session_state["mock2_tab"]="PLAYERS_AVAILABLE"; st.rerun()
        return
    ...
    tab=str(st.query_params.get("draft_tab") or st.session_state.get("mock2_tab","PLAYERS_AVAILABLE")).upper()
    aliases={"TEAM":"ROSTER","RESULTS":"DRAFT_BOARD"}
    tab=aliases.get(tab,tab)
    tab=tab if tab in {"PLAYERS_AVAILABLE","QUEUE","DRAFT_BOARD","ROSTER"} else "PLAYERS_AVAILABLE"
    st.session_state["mock2_tab"]=tab
    labels=(("PLAYERS_AVAILABLE","PLAYERS AVAILABLE"),("QUEUE","QUEUE"),("DRAFT_BOARD","DRAFT BOARD"),("ROSTER","ROSTER"))
    st.markdown('<div class="mock-tabs">'+''.join(f'<a class="mock-tab{" active" if tab==k else ""}" href="?page=Mock%20Draft&draft_tab={k}" target="_self">{v}</a>' for k,v in labels)+'</div>',unsafe_allow_html=True)
    if tab=="PLAYERS_AVAILABLE": _player_list(state,history,roi,rankings,weekly,ask_shiva_func,api_key)
    elif tab=="QUEUE": _queue(state)
    elif tab=="DRAFT_BOARD": _board(state)
    else: _roster(state)
    _status_bar(state)
