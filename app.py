...
# Context defaults are kept in session state; selectors are shown only
# inside the sections where league/manager context is actually relevant.
DEFAULT_SCOPE = "Combined"
default_managers = current_managers(DEFAULT_SCOPE)
preferred_manager = next(
    (
        candidate for candidate in ["Chris H", "Chris Hart"]
        if candidate in default_managers
    ),
    default_managers[0] if default_managers else "",
)

scope = st.session_state.get("active_scope", DEFAULT_SCOPE)
manager = st.session_state.get("active_manager", preferred_manager)
if manager not in current_managers(scope):
    available = current_managers(scope)
    manager = available[0] if available else preferred_manager
...
<div class="shiva-nav-shell">
  <div class="shiva-nav-title">SHIVA TOOLS</div>
</div>
""",
    unsafe_allow_html=True,
)
...
def contextual_league_manager_selector(
    key_prefix: str,
    initial_scope: str = "Combined",
) -> tuple[str, str, pd.DataFrame, str]:
    """Render league/manager controls only where they are useful."""
    scope_key = f"{key_prefix}_scope"
    manager_key = f"{key_prefix}_manager"

    selected_scope = st.selectbox(
        "League",
        ["Shiva", "Shiva 2.0", "Combined"],
        index=["Shiva", "Shiva 2.0", "Combined"].index(initial_scope),
        key=scope_key,
    )
    available_managers = current_managers(selected_scope)

    saved_manager = st.session_state.get(manager_key, preferred_manager)
    if saved_manager not in available_managers:
        saved_manager = available_managers[0] if available_managers else ""

    selected_manager = st.selectbox(
        "Manager",
        available_managers,
        index=(
            available_managers.index(saved_manager)
            if saved_manager in available_managers else 0
        ),
        key=manager_key,
    )

    st.session_state["active_scope"] = selected_scope
    st.session_state["active_manager"] = selected_manager

    selected_rows = franchise_rows(selected_manager, selected_scope)
    selected_team = franchise_name(selected_manager, selected_scope)
    return selected_scope, selected_manager, selected_rows, selected_team

if page == "League History":
    st.markdown(
        '<div class="section-label">League History Scope</div>',
        unsafe_allow_html=True,
    )
    history_scope, history_manager, rows, team_name = (
        contextual_league_manager_selector("history")
    )
...
""",
    ...
    st.markdown(
        '<div class="section-label">Draft Coach Profile</div>',
        unsafe_allow_html=True,
    )
    coach_scope, coach_manager, rows, team_name = (
        contextual_league_manager_selector("coach")
    )
...
""",
    ...
    st.markdown(
        '<div class="section-label">Personalized Draft Plan</div>',
        unsafe_allow_html=True,
    )
    plan_scope, plan_manager, rows, team_name = (
        contextual_league_manager_selector("plan")
    )
    slot = st.number_input(
        "Draft Position", 1, 10, 1, 1, key="draft_plan_slot"
    )
